    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center backdrop-blur-[1px] bg-black/60" onClick={handleClose}>
      <div className="bg-white rounded-lg shadow-lg w-full max-w-md p-6 relative" onClick={e => e.stopPropagation()}>
        ...
        <form className="space-y-4" onSubmit={handleSubmit}>
          ...
          <div>
            <label className="block mb-1 text-sm font-medium" htmlFor="role">Role</label>
            <select
              className="w-full border rounded px-3 py-2"
              id="role"
              value={role}
          ...
          <div>
            <label className="block mb-1 text-sm font-medium" htmlFor="departments">Departments</label>
            <select
              className="w-full border rounded px-3 py-2"
              id="departments"
              multiple
              value={departments}
              onChange={e =>
                setDepartments(Array.from(e.target.selectedOptions, option => Number(option.value)))
              }
              required
            >
